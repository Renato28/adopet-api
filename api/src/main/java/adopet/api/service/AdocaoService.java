...
public class AdocaoService {
    ...
    public List<AdocaoDTO> listarTodos() {
        ...
        return adocaoRepository.findAll().stream().map
                (AdocaoDTO::new).toList();
    }

    public AdocaoDTO listar(Long id) {

        return adocaoRepository.findById(id).stream().findFirst().map
                (AdocaoDTO::new).orElse(null);
    }
    ...
    public void solicitar(SolicitacaoDeAdocaoDTO dto) {
        ...
        Boolean petAdocaoEmAndamento = adocaoRepository.existsByPetIdAndStatus(dto.idPet(), Status.AGUARDANDO_AVALIACAO);
        ...
        Integer tutorAdocoes = adocaoRepository.countByTutorIdAndStatus(dto.idTutor(), Status.APROVADO);
